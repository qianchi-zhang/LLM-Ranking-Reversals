...
\begin{abstract}
Large language model leaderboards usually present benchmark performance as a single ordered list, implicitly assuming that the ordering is stable under reasonable evaluation choices. This project tests that assumption by studying whether non-semantic prompt perturbations and evaluation-subset variation can change model rankings. We evaluate four models, GPT-4o-mini, Gemini-2.0-flash, Qwen-2.5-7B, and Llama-3.1-8B, on MMLU, ARC-Challenge, and HellaSwag. The main standardized experiment uses 900 questions, four task-preserving prompt templates, and 14,400 model responses. We then compute accuracy, prompt-wise ranking reversal rate (PRRR), subset ranking reversal rate (SRRR), and supplementary rank-probability summaries using 1,000 half-subset resampling iterations per scope. To probe finer-grained instability, we also run an auxiliary expanded MMLU subject-level extension over five subjects with 200 questions per subject. The central finding is that ranking stability is highly task-dependent: ARC-Challenge is comparatively robust, MMLU has a stable top model but less stable lower ranks, and HellaSwag is substantially more fragile. At the subject level, instability can be even stronger, with high-school world history reaching a maximum PRRR of 74.30\% and sociology reaching a maximum SRRR of 67.90\%. These results show that leaderboard interpretation should distinguish average accuracy from ranking robustness.
\end{abstract}
...
\begin{definition}[Accuracy and Ranking]
    ...
    \]
    where $s(x,T,M)\in\{0,1\}$ is the fixed binary correctness score for model $M$ on item $x$ under template $T$. Further, all models (four models in our experiment) can be sorted by their accuracy on $A$ under $T$, inducing a ranking, denoted $R_{A,T}$.
\end{definition}

\textbf{Prompt-wise Ranking Reversal Rate (PRRR)}: For an item set $P$, two templates $T_1$ and $T_2$, draw $N$ subsets $P_i \subset P$; the PRRR on $P$ between $T_1$ and $T_2$ is the frequency of different rankings occuring under two templates among the $N$ subsets, denote PRRR($P, T_1, T_2$). i.e.,
    $$ \text{PRRR}(P, T_1, T_2):= \# \{i \in [1:N] : R_{P_i, T_1} \neq R_{P_i, T_2}\} / N.$$
     \textbf{Subset Ranking Reversal Rate (SRRR)}: For an item set $P$ and template $T$, draw $N$ subsets $P_i \subset P$; the SRRR on $P$ under $T$ is the frequency of different rankings occuring under $T$ when asked subsets and the full set, denote SRRR($P, T$). i.e.,
